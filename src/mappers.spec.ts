...
describe("Mappers", () => {
  ...
  describe("mapCommands", () => {
    it("should map every provided commands into the given action creator", async () => {
      const fakeAction1 = { type: "ACTION1" };
      const fakeAction2 = { type: "ACTION2", value: 42, name: "foobar" };

      const fakeCommands = [
        jest.fn(() => Promise.resolve(fakeAction1)),
        jest.fn(() => Promise.resolve()),
        jest.fn(() => Promise.resolve(fakeAction2)),
      ];

      const fakeActionCreator = (
        action: typeof fakeAction1 | typeof fakeAction2,
      ) => ({
        action,
        type: "CHILD_ACTION",
      });

      const mappedCommands = mapCommands(fakeActionCreator, fakeCommands);

      const received = await Promise.all(
        mappedCommands.map(command => command()),
      );

      expect(received).toEqual([
        fakeActionCreator(fakeAction1),
        undefined,
        fakeActionCreator(fakeAction2),
      ]);
    });
  });
});
